...
int sopro(int i, int j, int jogador,int dir,char tipo){
    ...
    if(tipo=='P'){//tipo do jogador P ou B
        //se o tipo do jogador for 1 e a casa atual for do tipo 2=Boud então eu verifico se a proxima casa na mesma direção é vazia se for isso é uma jogada obrigatoria
          if(jogador==1 && (tab[i][j]=='B'||tab[i][j]=='d')){
            if(tab[i+dirP[dir][0]][j+dirP[dir][1]]==' '){//verificando proxima casa na mesma direção
          ...
          int w;
          for(w=0;w<2;w++){//Casa do tipo P eu verifico os arredores e se for B ou d eu entro e entro no caso acima pela recursão
            if(tab[i+dirP[w][0]][j+dirP[w][1]]=='B'||tab[i+dirP[w][0]][j+dirP[w][1]]=='d'){
                sopro(i+dirP[w][0], j+dirP[w][1],1,w,tipo);
            }
          }
    //Os proximo else if e else são iguais ao anterior só muda o tipo de peça B D ou d
    }else if(tipo=='B'){
...
void colocarPecas(){
    int i,j;
    //colocando as peças B ou P no tabuleiro
    for(i=0;i<8;i++){
            for(j=0;j<8;j++){
...
int verificarJogadas(int x, int y,int x1, int y1){
    //Verificando se as posições não saem do tabuleiro
    if(x<0||x>=8||y<0||y>=8||x1<0||x1>=8||y1<0||y1>=8){
       printf("Jogada invalida\n");
       return 0;
    }
    //verificando se a distancia de uma posição pra outra não é maior do que 2
    if(!((x-x1)*(x-x1)==(y-y1)*(y-y1)&&((x-x1)<=2||(x-x1)>=-2))){
        printf("Jogada invalida\n");
        return 0;
    }
    //verificando se a posição destino não é uma casa ocupada
    if(tab[x1][y1]!=' '){
        printf("Jogada invalida\n");
        return 0;
    }
...
int movimento(int x, int y,int x1, int y1, int jogador){
    ...
    if(jogador==1){
        if(tab[x][y]=='P'){//Caso peça P
            for(w=0;w<2;w++){//Verifico todas as jogadas validas pra essa peça e vejo se alguma é igual ao destino x1 e y1
                if(tab[x+dirP[w][0]][y+dirP[w][1]]=='B'){//caso as casas vizinhas sejam uma peça oposta eu verifico se tem uam espaõ vazio dps se tiver e essa for a posição desejada a peça vai ser comida
            ...
            }//Mesma coisa do caso anterior so que vai ter 4 opções de jogada pq é uma dama
        }else if(tab[x][y]=='D'){
            for(w=0;w<4;w++){
...
void chamarSopro(int turno){
    indexJogadas=0;
    //varrendo todos o tabuleiro e verificando se naquela peça existe jogada obrigatoria
    int i,j;
    if(turno==1){
        for(i=0;i<8;i++){
            for(j=0;j<8;j++){
...
void jogar(){
    int x,y,x1,y1;
    //turno inicial é com as peças p
    int turno = 1;
    //limpando a tela
    system("cls");
    //inicializando o tabuleiro e colocando peças
    iniciarTab();
    colocarPecas();
    printarTab();
    int i,j;
...
int main()
{
    //Alocando espaõ pro nomes dos jogadores
    ...
    while(1){
        ...
        printf("4-Sair\n");
        scanf(" %c",&menu);
        //opções do menu que iram se repetir até ser alguma valida
        if(menu=='1'){
